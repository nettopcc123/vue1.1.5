  ...
  <div class="newlists" id="newlists">
        ...
        <ul class="nlist">
            <li v-for="(value, key) in newList" v-if="(key == 3)">
               <span class="nimg_b"><img :src="require('./../components/img/ban/d/' + value.Image +'.jpg')"></span>
                <router-link :to="{ name: 'newsMore1', params: { articid: value.Id , page: num, content: value.Content}}">
                <span class="newsCtn_b">
                    <h2 class="ntit">{{ value.Title | filter40 | restr }}</h2>
                    <p class="ntb"><span class="nname">消息来源： {{ value.Source }} </span></p>
                    <i class="mark2">{{ value.username }}</i>
                </span>
        ...
        <ul class="nlist">
            <li v-for="(value, key) in newList" v-if="(key > 4)">
                <router-link :to="{ name: 'newsMore1', params: { articid: value.Id , page: num, content: value.Content}}">
                <span class="nimg"><img :src="require('./../components/img/ban/i/' + value.Image +'.jpg')"></span>
                <span class="newsCtn">
      ...
      newVue:function(num){
        ...
        axios.interceptors.response.use(response => {
           ...
           // this.$store.commit('isloadhid');
            ...
            axios.get('http://154.48.238.35:8085/UserService.svc/NewsList?newstype=财经&pageindex=' + num + '&pagesize=10')  /// http://www.hd.me/data.php?callback=dosomething    static/news.json?num  static/news.json  http://misc.opencai.net/consts/lotts.json   /static/news.json
            .then(res => {
              console.log(res)
